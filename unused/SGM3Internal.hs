...
trainAndClassify s p
  | size s < capacity s && size s < 2        = addModelTrainAndClassify s p
  | size s < capacity s && bmuDiff > cutoff  = addModelTrainAndClassify s p
  | size s == capacity s && bmuDiff > cutoff = (bmu4, bmuDiff, report4, s4)
