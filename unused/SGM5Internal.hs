------------------------------------------------------------------------
-- |
-- Module      :  Data.Datamining.Clustering.SGM5Internal
...
module Data.Datamining.Clustering.SGM5Internal where
...
-- existing models or update the model counts.
classify'
  :: (Num t, Ord t, Num x, Ord x, Enum k, Ord k)
    => SGM t x k p -> p -> (k, x, M.Map k (p, x), SGM t x k p)
classify' s p = (bmu, bmuDiff, report, s')
  where s' | numModels s < maxSize s = addNode p s
           | otherwise               = s
        (bmu, bmuDiff, report) = classify s' p
...
trainAndClassify
  :: (Num t, Ord t, Num x, Ord x, Enum k, Ord k)
    => SGM t x k p -> p -> (k, x, M.Map k (p, x), SGM t x k p)
trainAndClassify s p = (bmu, bmuDiff, report, s4)
  where (bmu, bmuDiff, report, s2) = classify' s p
        s3 = trainNode s2 bmu p
        s4 = incrementCounter bmu s3
