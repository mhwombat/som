...
This program builds a Dynamic Self-Organising Map (DSOM) based on the
colours in an image. As the DSOM learns the colours in the image, it
builds a map of the colour distribution of the image. The map here is a
hexagonal grid with 7 tiles in it, so it divides the colours into 7
clusters. The value associated with each tile is the RGB value of the
colour for that cluster.
...
main = do
  ...
  c <- evalRandIO $ buildDSOM :: IO (DSOM (LGridMap HexHexGrid) k Pixel)
...
readPixels f = do
  ...
  evalRandIO . shuffle . concat . replicate 10 . img2vectors $ img

-- Build a classifier initialised with random values.
buildDSOM :: RandomGen r => Rand r (DSOM (LGridMap HexHexGrid) k Pixel)
buildDSOM = do
...
instance Pattern Pixel where
  ...
  difference x y = (euclideanDistanceSquared x y) / (3*(255^2))
