------------------------------------------------------------------------
-- |
-- Module      :  Data.Datamining.Clustering.SGM5QC
...
import           Data.Datamining.Clustering.SGM5Internal
...
prop_models_not_deleted
  :: TestSGM -> Double -> Property
prop_models_not_deleted (TestSGM s _) x =
  property $ null (labelsBefore \\ labelsAfter)
  where labelsBefore = M.keys $ modelMap s
        labelsAfter = M.keys $ modelMap s'
        (_, _, _, s') = trainAndClassify s x

prop_models_not_deleted2
  :: TestSGM -> Double -> Property
prop_models_not_deleted2 (TestSGM s _) x =
  property $ null (labelsBefore \\ labelsAfter)
  where labelsBefore = M.keys $ modelMap s
        labelsAfter = M.keys $ modelMap s'
        s' = train s x
...
test :: Test
test = testGroup "QuickCheck Data.Datamining.Clustering.SGM5"
  [
    ...
    testProperty "prop_models_not_deleted"
      prop_models_not_deleted,
    testProperty "prop_models_not_deleted2"
      prop_models_not_deleted2
  ]
