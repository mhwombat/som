...
This program builds a Dynamic Self-Organising Map (DSOM) based on some
(fake) housing prices. As the DSOM learns the data, it builds a map of
the data. The map here is a square grid with 4 tiles in it, so it
divides the prices into 4 clusters. The value associated with each tile
is the model ("typical" price) of the houses in that cluster.
-}
...
import Data.Datamining.Clustering.DSOM (DSOM(..), defaultDSOM, toGridMap)
...
main :: IO ()
main = do
  -- Build a self-organising map (DSOM) initialised with small random values.
  let c = buildDSOM
  print c
  foldM_ trainAndPrint c inputData

trainAndPrint :: DSOM (LGridMap RectSquareGrid) (Int, Int) Price -> Price -> IO (DSOM (LGridMap RectSquareGrid) (Int, Int) Price)
...
-- Build a classifier initialised with values that span the range.
buildDSOM :: DSOM (LGridMap RectSquareGrid) (Int, Int) Price
buildDSOM = defaultDSOM gm 0.1 1.6
 where g = rectSquareGrid 1 5  -- The grid we'll use for our DSOM
       ps = [minPrice, minPrice+delta..]
       delta = (maxPrice-minPrice)/((fromIntegral . tileCount $ g) - 1)
       gm = lazyGridMap g ps -- a map from grid positions to prices

type Price = Double

instance Pattern Price where
  type Metric Price = Double
  difference x y = abs (x - y) / (maxPrice - minPrice)
  makeSimilar x amount y = y + amount*(x - y)

instance Show (DSOM (LGridMap RectSquareGrid) (Int, Int) Price) where
...
inputData :: [Price]
inputData = concat . replicate 500 $ [72160,151100,351600,350100,347700,149500,349500,73100,
