...
main = do
  ...
  let c = buildSOM (length inputData)
  print . toGridMap $ c
...
-- Build a classifier using initial values that
-- are spread evenly over the expected range of input values.
buildSOM :: Int -> SOM (LGridMap RectSquareGrid) k Price
buildSOM n = defaultSOM gm 0.5 0.1 0.3 0.1 n
  where g = rectSquareGrid 1 4  -- The grid we'll use for our SOM
        ps  = [0, (maxPrice/4)..maxPrice]
